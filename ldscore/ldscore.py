...
def getBlockLefts(coords, max_dist):
    ...
    将坐标+最大块长度转换为要包含在块中最左边的SNP的坐标列表。
...
def block_left_to_right(block_left):
    ...
    ----------
    block_left : array
        Array of block lefts.       什么是block? block left又是什么鬼
    ...
    M = len(block_left)
    j = 0
    block_right = np.zeros(M)  # np是numpy, np.zeros(M)是生成长度为M的零矩阵
    for i in xrange(M):  # 生成数组 [0,1,2,……,M-1]
...
class __GenotypeArrayInMemory__(object):
    ...
    是包含不同基因型矩阵文件，比如 plink，.bed文件等接口的类的父类
    '''
