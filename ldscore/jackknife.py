...
of the data.
这个模块中都是来处理2维数组的。1维数据被表示N行1列的数组或者1行n列的数组，避免由numpy将(N,)视作与（N，1）
不同而引起的bug
'''
...
class Jackknife(object):
    ...
    JackKnife对象的基类。输入包含x，y，因此这个基类是为了由自变量和因变量计算数据（比如回归）而量身定做的
    ...
    *args, **kwargs :
    ...
    def __init__(self, x, y, n_blocks=None, separators=None):
        ...
        if separators is not None:
            ...
            # 为什么要减一？因为separators包含两边界，块是两个边界之间的
    ...
    def jknife(cls, pseudovalues):
        ...
        # 求协方差矩阵
        jknife_cov = np.atleast_2d(np.cov(pseudovalues.T, ddof=1) / n_blocks)  # 为什么除以n_blocks？？ddof=1 是要标准化
        # 协方差的对角线元素就是方差
        jknife_var = np.atleast_2d(np.diag(jknife_cov))
        # 方差开平方得到标准差
        jknife_se = np.atleast_2d(np.sqrt(jknife_var))
        # 计算伪值矩阵每一列的均值，作为jackknife估计值
